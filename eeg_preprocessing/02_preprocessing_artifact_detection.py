...
# path to eeg files
data_path = os.path.join(root_path, 'derivatives/extract_blocks')
# path for output
derivatives_path = os.path.join(root_path, 'derivatives')

# create directory for save
if not os.path.isdir(os.path.join(derivatives_path, 'artifact_detection')):
    os.mkdir(os.path.join(derivatives_path, 'artifact_detection'))

output_path = os.path.join(derivatives_path, 'artifact_detection')
...
# from MNE-Python (https://github.com/mne-tools/mne-python/blob/master/mne/preprocessing/bads.py);  # noqa
...
for file in files:
    ...
    picks_no_eeg = mne.pick_types(raw.info,
                                  eeg=False,
                                  eog=True,
                                  stim=True)
    ...
    ch_ix = [raw_copy.info['ch_names'].index(chan) for chan in raw_copy.info['ch_names'] if chan not in ignore_ch]  # noqa
    ...
    frequency_of_annotation = {x: annotated_channels.count(x) for x in annotated_channels}  # noqa
    ...
    # save file
    raw_copy.save(os.path.join(output_path, subj, '%s_artifact_detection-raw.fif' % subj),  # noqa
                  overwrite=True)
    ...
    sfile.write(str(round(sum(duration) / (raw_copy._last_time - raw_copy._first_time), 3)) + ' %\n')  # noqa
    sfile.close()
